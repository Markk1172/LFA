...
É um conjunto finito e não vazio de símbolos indivisíveis caracteres usados para formar palavras.

* [ ] O que é uma **cadeia**;
É uma sequência finita de símbolos escolhidos de um determinado alfabeto, também chamada de palavra ou string.

* [ ] O que significa **`ε`**;
Representa a cadeia vazia, ou seja, uma palavra que não contém absolutamente nenhum símbolo do alfabeto.

* [ ] Por que **`|ε| = 0`**;
As barras indicam o comprimento (número de símbolos) da cadeia, e como a cadeia vazia não tem símbolos, seu tamanho é zerp

* [ ] O que é um **prefixo**;
É qualquer sequência de caracteres que forma a parte inicial de uma cadeia, podendo ir da cadeia vazia até a palavra inteira.

* [ ] O que é um **sufixo**;
É qualquer sequência de caracteres que forma a parte final de uma cadeia, podendo ir da cadeia vazia até a palavra inteira.

* [ ] O que significa **`Σ*`**;
Conhecido como Fecho de Kleene, é o conjunto de todas as cadeias possíveis que podem ser formadas com o alfabeto $\Sigma$, incluindo a cadeia vazia ($\epsilon$).

* [ ] Se `Σ*` possui limite de tamanho;
Não possui limite. O conjunto $\Sigma^*$ é infinito porque as combinações e os tamanhos das cadeias que podem ser criadas são ilimitados.

* [ ] O que é uma **linguagem formal `L`**;
É um conjunto específico de cadeias (palavras) geradas a partir de um alfabeto, definidas por um conjunto rigoroso de regras matemáticas ou estruturais.

* [ ] O que significa **`L ⊆ Σ*`**;
Significa que a linguagem formal $L$ é um subconjunto de $\Sigma^*$, ou seja, todas as palavras de $L$ são formadas por cadeias válidas daquele alfabeto.

* [ ] O que é uma **gramática formal**;
É um sistema de regras de substituição precisas usado para definir e gerar todas as cadeias válidas que pertencem a uma linguagem formal.

* [ ] O que são **terminais e não terminais**;
Terminais são os símbolos finais e definitivos de uma palavra (do alfabeto $\Sigma$). Não terminais são variáveis auxiliares que ainda devem ser substituídas por outras regras.

* [ ] O que é uma **regra de produção**;
É uma instrução matemática que dita como um símbolo não terminal pode ser substituído por uma combinação de símbolos terminais e não terminais.

* [ ] Como ler **`S → aS | ε`**;
 "A variável não terminal $S$ produz o terminal 'a' seguido do próprio $S$, ou produz a cadeia vazia $\epsilon$".

* [ ] Como gerar palavras usando uma gramática.
começa pelo símbolo inicial (geralmente $S$) e aplica as regras de produção repetidamente até que a cadeia final contenha apenas símbolos terminais.
